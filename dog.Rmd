...
```{r}
pre.pairs <- read.delim("pre_exercise_pairs_bray_curtis.tsv")
...
ggsave("figures/Boxplots/pre-pairwise.jpg", pre.pairs.plot, device="jpeg", dpi=300)
```


```{r}
post.pairs <- read.delim("post_exercise_pairs_bray_curtis.tsv")
...
post.pairs.plot <- ggplot(post.pairs, aes(x=Group, y=Distance, fill=Group)) + 
    stat_boxplot(geom = "errorbar", width = 0.2) +  
    geom_boxplot(outlier.shape = 1, outlier.size = 5, outlier.stroke = 1) +
    geom_signif(xmin=c(1,1,2.1), xmax=c(3,1.9,3), y_position = c(0.93,0.85, 0.85), annotations = c("**","**","†"), map_signif_level = TRUE, textsize=10, vjust=-0.1) +
    theme_bw() +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank()) +
    theme(axis.line = element_line(colour = "black")) +
    theme(legend.position="none", text = element_text(size = 30, family="arial"), axis.text = element_text(size=30)) +
...
ggsave("figures/Boxplots/post-pairwise.jpg", post.pairs.plot, device="jpeg", dpi=300)
```
...
```{r}
yes.pairs <- read.delim("yes_glucosamine_pairs_bray_curtis.tsv")
...
ggsave("figures/Boxplots/yes-pairwise.jpg", yes.pairs.plot, device="jpeg", dpi=300)
```

```{r}
no.pairs <- read.delim("no_glucosamine_pairs_bray_curtis.tsv")
...
ggsave("figures/Boxplots/no-pairwise.jpg", no.pairs.plot, device="jpeg", dpi=300)
```
...
lacto.plot <- ggplot(df.lacto, aes(x=Group, y=Taxon, fill=Group)) + 
    ...
    scale_y_continuous(breaks=c(0,0.35,0.7),
                       labels=c("0","0.35","0.7"),
                       limits=c(0,0.7)) +
    xlab("Subgroup") +
    ylab(expression(paste(italic("Lactobacillaceae"), " RA")))

ggsave("figures/TaxaAbundancePlots/lacto.jpg", lacto.plot, device="jpeg", dpi=300, width = 8.5)
```
...
ggsave("figures/TaxaAbundancePlots/anaero.jpg", anaero.plot, device="jpeg", dpi=300, width=8.5)
```
...
brachy.plot <- ggplot(df.brachy, aes(x=Group, y=Taxon, fill=Group)) + 
    ...
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank()) +
    theme(axis.line = element_line(colour = "black")) +
    theme(legend.position="none", text = element_text(size = 30, family="arial"), axis.text = element_text(size=30), axis.title.y = element_text(size=30)) +
    scale_fill_grey(start=1, end=0.5) +
    scale_x_discrete(labels=c("PreNo","PreYes","PostNo","PostYes")) +
...
ggsave("figures/TaxaAbundancePlots/brachy.jpg", brachy.plot, device="jpeg", dpi=300, width=8.5)
```
...
ggsave("figures/TaxaAbundancePlots/sellimonas.jpg", sellimonas.plot, device="jpeg", dpi=300, width=8.5)
```
...
parvibacter.plot <- ggplot(df.parvibacter, aes(x=Group, y=Taxon, fill=Group)) + 
    stat_boxplot(geom = "errorbar", width = 0.2) +  
    geom_boxplot(outlier.shape = 1, outlier.size = 5, outlier.stroke = 1) +
    geom_signif(xmin=c(1,2,3.1), xmax=c(4,2.9,4), y_position = c(0.00096,0.00088,0.00088),
...
ggsave("figures/TaxaAbundancePlots/parvibacter.jpg", parvibacter.plot, device="jpeg", dpi=300, width=8.5)
```
...
uncultured.plot <- ggplot(df.uncultured, aes(x=Group, y=Taxon, fill=Group)) + 
    stat_boxplot(geom = "errorbar", width = 0.2) +  
    geom_boxplot(outlier.shape = 1, outlier.size = 5, outlier.stroke = 1) +
    geom_signif(comparisons = list(c("no - pre", "yes - post"), c("no - post", "yes - post")), y_position = c(0.0005,0.0004),
                annotations = c("*","*"), map_signif_level = TRUE, textsize=13, vjust=0.5) +
    theme_bw() +
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_blank()) +
    theme(axis.line = element_line(colour = "black")) +
...
ggsave("figures/TaxaAbundancePlots/uncultured.jpg", uncultured.plot, device="jpeg", dpi=300, width=8.5)
```
